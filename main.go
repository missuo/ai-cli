...
func promptProvider(in *os.File, errOut *os.File) (string, error) {
	...
	for {
		...
		if name == "" {
			fmt.Fprintln(errOut, "Please choose 1/claude or 2/codex.")
			continue
		}
...
func missingAllProvidersError() error {
	return errors.New("no supported AI agent was found in PATH. Install Claude Code (`claude`) or Codex (`codex`) and try again")
}
...
When no provider is supplied, ai prompts for Claude or Codex, then passes every
remaining argument to the selected command.
